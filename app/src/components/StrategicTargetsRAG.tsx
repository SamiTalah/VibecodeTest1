...
export type StatusKey = 'On track' | 'At risk' | 'Not on track' | 'Done' | 'On hold' | 'TBD';
...
type Totals = Record<StatusKey, number>;
...
type Store = Record<string, Record<string, CardData[]>>;
...
const SEVERITY_ORDER: StatusKey[] = ['Not on track', 'At risk', 'On track', 'Done', 'On hold', 'TBD'];

// HELPERS
const sum = (obj: Partial<Totals> | undefined): number => Object.values(obj || {}).reduce((a, b) => a + b, 0);
...
function rollupObjective(statuses: string[]): StatusKey {
...
function parseTable(text: string) {
  ...
  const out: Array<{ year: string; pi: string; target: string; objective: string; rag: StatusKey }> = [];
...
function aggregateCounts(rows: ReturnType<typeof parseTable>, year: string, pi: string): CardData[] {
  ...
  const objectiveRAG: Array<{ target: string; objective: string; rag: StatusKey }> = [];
  ...
  const byTarget = new Map<string, Totals>();
...
const MiniKPIs: React.FC<{ totals: Partial<Totals> }> = ({ totals }) => {
  const total = sum(totals);
  const ok = ((totals['On track'] || 0) as number) + ((totals['Done'] || 0) as number);
  const sev = (totals['Not on track'] as number) || 0;
  return (
    <div className="grid grid-cols-3 gap-3 mt-4">
      ...
      <div className="rounded-2xl bg-amber-50 p-3">
        ...
        <div className="text-xl font-semibold text-amber-900">{(totals['At risk'] as number) || 0}</div>
      </div>
...
const CompactLegend: React.FC<{ totals: Partial<Totals> }> = ({ totals }) => {
  const total = sum(totals);
  return (
...
const TargetCard: React.FC<{ target: string; totals: Totals }> = ({ target, totals }) => {
  const total = sum(totals);
...
export default function StrategicTargetsRAG() {
  const [store, setStore] = useState<Store>(DEFAULT_COUNTS);
  const years = Object.keys(store).sort();
  const [year, setYear] = useState<string>(years[years.length - 1] || '2025');
  const pis = Object.keys(store[year] || {}).sort();
  const [pi, setPI] = useState<string>(pis.includes('PI3') ? 'PI3' : (pis[0] || 'PI3'));

  const cards = useMemo<CardData[]>(() => (store[year] && store[year][pi]) ? store[year][pi] : [], [store, year, pi]);
  const grand = useMemo<Totals>(() => {
    const acc: Totals = { 'On track': 0, 'At risk': 0, 'Not on track': 0, 'Done': 0, 'On hold': 0, 'TBD': 0 };
    for (const c of cards) for (const k of Object.keys(acc) as StatusKey[]) acc[k] += c.totals[k] || 0;
    return acc;
  }, [cards]);
  ...
  function handleIngest(text: string) {
    try {
      ...
      for (const pair of pairs) {
        ...
        if (!next[y]) next[y] = {} as Record<string, CardData[]>;
        next[y][p] = aggregated;
      }
      setStore(next);
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <details className="ml-2">
              ...
              <div className="mt-2 p-3 rounded-xl border border-slate-200 bg-white max-w-xl">
                ...
                <pre className="text-xs bg-slate-50 p-2 rounded-md overflow-auto">{`year\tpi\tstrategic target\tobjective\trag
2025\tPI2\t24/7 availability\tStable, secure...\tOn track
2025\tPI2\tSolid risk management & compliance\tEnsure future regulatory compliance\tAt risk`}</pre>
                ...
                <div className="mt-2 flex justify-end">
                  <button
                    ...
                    onClick={() => {
                      const el = document.getElementById('ingest-text') as HTMLTextAreaElement | null;
                      if (el) handleIngest(el.value);
                    }}
                  >Import</button>
                </div>
              </div>
