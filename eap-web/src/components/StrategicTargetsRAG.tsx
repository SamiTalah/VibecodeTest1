...
// STATUS TOKENS
const STATUS: Record<string, { label: string; bg: string; fg: string; bar: string; icon: React.ReactNode }> = {
...
const SEVERITY_ORDER = ["Not on track", "At risk", "On track", "Done", "On hold", "TBD"];

const sum = (obj: Record<string, number> | undefined) => Object.values(obj || {}).reduce((a, b) => a + b, 0);
...
function rollupObjective(statuses: string[]): string {
...
const DEFAULT_COUNTS: Record<string, Record<string, Array<{ target: string; totals: Record<string, number> }>>> = {
  2025: {
    PI3: [
...
function parseTable(text: string) {
  ...
  const out: Array<{ year: string; pi: string; target: string; objective: string; rag: string }> = [];
...
function aggregateCounts(rows: Array<{ year: string; pi: string; target: string; objective: string; rag: string }>, year: string, pi: string) {
  const filtered = rows.filter((r) => String(r.year) === String(year) && String(r.pi).toUpperCase() === String(pi).toUpperCase());
  if (filtered.length === 0) return [] as Array<{ target: string; totals: Record<string, number> }>;
  ...
  const objectiveRAG: Array<{ target: string; objective: string; rag: string }> = [];
...
const StackedBar: React.FC<{ totals: Record<string, number> }> = ({ totals }) => {
...
const MiniKPIs: React.FC<{ totals: Record<string, number> }> = ({ totals }) => {
...
const CompactLegend: React.FC<{ totals: Record<string, number> }> = ({ totals }) => {
  const total = sum(totals);
  return (
...
const TargetCard: React.FC<{ target: string; totals: Record<string, number> }> = ({ target, totals }) => {
  const total = sum(totals);
  const headline = (totals["Not on track"] || 0) > 0
...
export default function StrategicTargetsRAG() {
  ...
  const grand = useMemo(() => {
    const acc: Record<string, number> = { "On track": 0, "At risk": 0, "Not on track": 0, "Done": 0, "On hold": 0, "TBD": 0 };
    for (const c of cards) for (const k of Object.keys(acc)) acc[k] += c.totals[k] || 0;
    return acc;
  }, [cards]);
  const allObjectives = sum(grand);
  const headerPulse = (grand["Not on track"] || 0) > 0 ? "Portfolio attention required" : (grand["At risk"] || 0) > 0 ? "Monitor and unblock quickly" : "Tracking to plan";

  function handleIngest(text: string) {
    try {
      const rows = parseTable(text);
      const pairs = Array.from(new Set(rows.map(r => `${r.year}||${r.pi}`)));
      const next = { ...store } as Record<string, Record<string, Array<{ target: string; totals: Record<string, number> }>>>;
      for (const pair of pairs) {
        const [y, p] = pair.split("||");
        const aggregated = aggregateCounts(rows, y, p);
        if (!next[y]) next[y] = {} as any;
        next[y][p] = aggregated as any;
      }
      setStore(next);
